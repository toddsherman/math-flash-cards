...
- Three equal-sized buttons sit at the bottom right of each card: check on top, X in the middle, and speaker at the bottom. The check plays a cheerful chime; the X plays a short, sharp buzzer. Feedback keeps the current card in place. Feedback starts on press using prebuilt Web Audio buffers, avoiding file decoding and media seeking on each tap. The chime is defined in `lib/feedback-audio.ts`. The X uses [this supplied buzzer clip](https://www.youtube.com/watch?v=FRpq7o1mKXY), with 0.609 seconds trimmed from the beginning. Its trimmed WAV is stored at `public/audio/feedback/incorrect-youtube-v1.wav`; `python3 scripts/embed-buzzer.py` embeds the PCM samples in `lib/incorrect-sound.json` for immediate playback. The older WAV files and Python generator preserve the original sound design.
